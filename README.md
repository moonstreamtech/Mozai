...
src/
  ...
  placeholder-grid.ts   Temporary canvas grid (paint scene placeholder)
  ...
  style.css             #game / #banner / --banner-h + scene chrome
  content.ts            Content-index + per-puzzle JSON loader
  progress.ts           Capacitor Preferences-backed completion store
  scenes.ts             Minimal scene manager (rooms | room | paint)
  scene-rooms.ts        Room-select grid (5 cols, % fill, lock rule)
  scene-room.ts         Room-interior stub (picture list)
  scene-paint.ts        Paint scene stub (mark-complete debug button)
index.html              Viewport meta + safe-area-aware DOM shell
capacitor.config.ts     appId com.moonstreamtech.mozai
scripts/
  build-content-index.mjs  Prebuild — scans public/content -> index.json
public/content/
